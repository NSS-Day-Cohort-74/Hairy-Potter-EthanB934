...
1. Explain how you got the HTML, with the correct data, displayed in the browser?
   > The objects were stored in an array in PotteryCatalog after they met the requirements of the conditions therein. The stored array of qualifying objects was then exported by function into PotteryList so that I could work with the new, filtered array of objects. When first creating the function, I had to create an empty string and store it in a variable called "html" and declared it with the let keyword so that it could later be reassigned the value of the HTML in the terminal string. I had to use a for of loop to iterate through the array of objects, item by item. When one object was pulled from the array of objects and stored in a single variable, in this case "potteryPiece," I could access that objects's properties. I used a terminal string to have more flexibility while working with the HTML. Finally, after the for of loop ran, we commit the reassignment of the empty string to now include the terminal string of HTML text and the interpolations of the "potteryPieces's" properties. This is then returned to the main module and stored in a new variable called "html" and declared with const, since I would not have to modify the value any further. I target the article class tag because that is where I planned to inject the created and stored HTML. Once the class was targeted, I used the .innerHTML method on the value returned by the targeting. The .innerHTML can read and translate HTML code found in our stored value returned from the PotteryList module. 
2. In the **PotteryList** module, when you iterate your pottery, you need to show the evidence of what the **weight** property's value is for the 2nd piece of pottery. Use [Loom](https://www.loom.com/) to record your browser window with the developer tools open and show those values.
   > [Paste your video's public URL here](https://www.loom.com/share/944466f05235432d94d7d47d81dd04cd?sid=4e950ca1-0612-4b5c-8241-87552fa3983d)
3. The **PotteryWheel** module has a single function named `makePottery`. Why doesn't that module have all of the other code in it?
   > For our code to remain modular. We want easy-to-read code and ideally for each module to have single responsibility; that is, each module is concerned with only one part of the whole project. Having modular code may also help isolate bugs. If all of the code was kept in one module, debugging multiple function calls, scrolling up and down the module is hard-to-follow. Having a module care for one part of the whole project helps diagramming the plan for building the code. 
4. The pottery shop has learned that there is a set of customers that are willing to buy cracked pottery at a discounted price of $2.50. That means that the cracked pottery should now be displayed in the catalog. Explain the changes that this new business strategy would cause to your algorithm.
   > In the algorithm, we have only one module that checks the properties of each object. That module is the potteryCatalog. As it currently sits, it excludes any cracked pottery. So, if our client is wanting to sell the cracked pottery at a discounted price, does our restrictions on the objects matter and do we need to keep the array into which we are pushing qualifying objects? Yes, we have to keep the entire module, but we need to refactor the restrictions we have on any object whose cracked property evaluates to true. The potteryList function would push the cracked objects with a new price property of 2.50 as the client specified. We would still need to keep the array as well, we must remember the purpose of the array is not served in this module, but in the potteryList module where our HTML is generated. We would need to generate HTML for any of the objects with the cracked property and possibly add additional HTML for their specific case of being cracked. 
5. In the **Kiln** module, you have a `firePottery()` function. You need to demonstrate how to use the debugger to verify the values of the parameters for that function when your code runs. Use [Loom](https://www.loom.com/) to record your browser window with the developer tools open and show those values.
   > [Paste your video's public URL here](https://www.loom.com/share/7d56872938d045bda700fa0421964f33?sid=5837103b-4f0a-4fa0-8ad8-cd629c6d5cfb)
